 ...
 *
 * The multifunctionalMapper function that accepts a list of operators (mappers) and returns a new operator. The new operator
 * just accepts a list of integer numbers and sequentially applies each mapper to each number in the list
 * (performs multiple transformations). It produces a list of transformed values.
 * Based on multifunctionalMapper, implement the multTwoAndThenAddOneTransformation operator that multiplies each integer
 * number by two and then adds one to it. The operator is applied to each number in the input list.
 * Based on multifunctionalMapper, implement the squareAndThenGetNextEvenNumberTransformation operator that squares
 * each integer number and then calculates the next even number following it. The operator is also applied to each number
 * in the input list.
 */
class MultifunctionalMapper {
    ...
    public static final Function<List<IntUnaryOperator>, UnaryOperator<List<Integer>>>
            MULTIFUNCTIONAL_MAPPER = opList -> intList -> intList.stream()
            .map(i -> opList.stream()
